                ...
                <table class="table table-bordered  table-active table-dark-gray table-hover " style="width: 100%; padding:10px">
                    ...
                    <tbody>
                        @php
                            ...
                            $totalSum = 0;
                        @endphp
                        @forelse ($proposalItem->item as $item)

                            @php
                                $sum += $item->quantity * $item->price;
                            @endphp

                            @php $taxFix = $item->item->tax->rule ?? 0 ;  @endphp
        ...
        <div class="col-md-5">
            <div class="card summery">
                ...
                <div class="card-body proposalInfo">
                    ...
                    <h5 class="font-weight-bold">Company Name: {{$proposalItem->customers->company_name}}</h5><br>
                    <p>Address: {{$proposalItem->customers->address}}</p>
                    <p>Phone: {{$proposalItem->customers->phone}}</p>
                    <p>City: {{$proposalItem->customer->city ? $proposalItem->customers->city : 'NA'}}</p>
                    <p>Country: {{$proposalItem->customer->country ? $proposalItem->customers->country: 'NA'}}</p>
                    <p>Zip: {{$proposalItem->customer->zip ? $proposalItem->customers->zip : 'NA'}}</p>
                    <p>Vat Number: {{$proposalItem->customers->vat_number ?$proposalItem->customers->vat_number : 'Not Applicable'}}</p>
                    @if ($proposalItem->signText)
                        <p><strong>Signature: </strong><img src="{{$proposalItem->signText}}" alt="" style="width:120px"></p>
                    @endif
            ...
            <div class="btn button-list">
                ...
                @if ($proposalItem->status != 'accepted' && $proposalItem->status != 'rejected')
                ...
                @elseif($proposalItem->status == 'accepted')
                 ...
                 @elseif($proposalItem->status == 'rejected')
